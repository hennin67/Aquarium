...
*/
CFishAngel::CFishAngel(CAquarium* aquarium) : CItem(aquarium)
{
	mFishImage = unique_ptr<Bitmap>(Bitmap::FromFile(FishAngelImageName.c_str()));
	if (mFishImage->GetLastStatus() != Ok)
	{
		wstring msg(L"Failed to open ");
		msg += FishAngelImageName;
 ...
 */
void CFishAngel::Draw(Gdiplus::Graphics* graphics)
{
 ...
 * \return true if hit.
 */
bool CFishAngel::HitTest(int x, int y)
{
	double wid = mFishImage->GetWidth();
	double hit = mFishImage->GetHeight();
